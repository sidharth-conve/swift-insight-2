...
def solve_doubt(doubt: str) -> str:
    ...
    result = asyncio.run(intial_schema(doubt))
...
async def run(mcp_server: MCPServer, user_question:str) -> str:
    agent = Agent(
        name="Assistant",
    ...
    # print(output_format)
    schema_prompt =f'''Your task is to find important tables and columns to frame sql for question: '{user_question}'
...
async def intial_schema(user_question:str):
    ...
    async with MCPServerStdio(
        ...
        return await run(server, user_question)
...
def func1(user_question):
    schema = asyncio.run(intial_schema(user_question))
    print("-"* 50)
